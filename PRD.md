...
> **Branding:** Nama project resmi adalah **Pantau Pangan**. "Gelembung" boleh dipakai sebagai tagline visual (mis. *"Pantau Pangan — gelembung harga pangan strategis"*), tapi semua nama folder, package, database, dan domain pakai `pantau-pangan` / `pantaupangan.id`.
...
> **Catatan provinsi:** BI PIHPS belum mengikuti pemekaran wilayah Papua terbaru. Per Mei 2026, data BI hanya mengenal **34 provinsi** dengan Papua direpresentasikan sebagai *Papua* dan *Papua Barat* saja. Aplikasi mengikuti taksonomi BI apa adanya (sumber data adalah single source of truth) — tidak melakukan mapping/split sendiri ke provinsi pemekaran.
...
- *"Data terkini per {tanggal_terbaru}, {jam_scrape} WIB · Akumulasi sejak {tanggal_pertama}"* — transparan ke user soal kualitas & cakupan data.
...
- **Visualisasi:** D3.js (force simulation)
- **UI Components:** shadcn/ui
- **Styling:** Tailwind CSS
- **Data Fetching:** TanStack Query
- **Theme:** Dark / Light mode (shadcn built-in)

### 7.4 Scraper — `packages/scraper`
- **Runtime:** Bun fetch native — zero dependency, no Playwright
...
- Types: `Komoditas`, `HargaHarian`, `BubbleData`, `InsightResponse`
- Utils: kalkulasi % perubahan, normalisasi bubble size, warna bubble
- Constants: endpoint BI, threshold warna, daftar timeframe

### 7.6 Code Quality
- ESLint + Prettier
- Husky + lint-staged (pre-commit: lint + format changed files)
- commitlint — conventional commits (`feat:`, `fix:`, `chore:`, `docs:`)
- `tsc --noEmit` (pre-push hook)

### 7.7 Deploy
- **Frontend:** Vercel
- **Backend + Database:** Railway (PostgreSQL + Hono API dalam satu project)
...
|---|---|
| **M1 — Foundation** | Monorepo setup, Turborepo, ESLint/Prettier/Husky/commitlint, tsconfig |
| **M2 — Scraper** | Bun fetch → parse `GetDetailGridData2` → simpan ke PostgreSQL via Drizzle |
| **M3 — API** | Hono routes: `/komoditas`, `/historis`, `/detail`, `/insight`, `/provinsi` |
| **M4 — Bubble Chart** | D3.js force simulation, warna, ukuran, label, animasi, filter timeframe |
| **M5 — Modal Detail** | Chart historis, tabel geografis collapsible, LLM insight panel |
| **M6 — Polish** | Search, filter provinsi, dark/light mode, loading states, error states |
| **M7 — Deploy** | Railway (API + DB) + Vercel (Web), cron aktif, monitoring log |
